...
try{
  (async function ensureDefaultAdmin(){
    try{
      if (!admin) return;
      // If no Firebase project id or credentials are available, skip auto-creating
      // a default admin user. This avoids the GoogleAuth "Unable to detect a
      // Project Id" error in development when ADC or a service account is not
      // configured. Set `GOOGLE_APPLICATION_CREDENTIALS` (path to service
      // account JSON), `FIREBASE_CONFIG` (JSON string with projectId), or
      // `GCLOUD_PROJECT`/`GOOGLE_CLOUD_PROJECT` to enable admin operations.
      try {
        var detectedProjectId = null;
        try { detectedProjectId = (admin && admin.apps && admin.apps.length && admin.app().options && admin.app().options.projectId) || null; } catch(_) { detectedProjectId = null; }
        if (!detectedProjectId) {
          // Also try to detect a local service-account.json's project_id
          try{
            const localSa = path.join(__dirname, 'service-account.json');
            if(fs.existsSync(localSa)){
              try{ var sac2 = require(localSa); if(sac2 && sac2.project_id) detectedProjectId = sac2.project_id; }catch(e){}
            }
          }catch(e){}
          if (process.env.GCLOUD_PROJECT) detectedProjectId = process.env.GCLOUD_PROJECT;
          if (!detectedProjectId && process.env.GOOGLE_CLOUD_PROJECT) detectedProjectId = process.env.GOOGLE_CLOUD_PROJECT;
          if (!detectedProjectId && process.env.FIREBASE_CONFIG) {
            try { var _fc = JSON.parse(process.env.FIREBASE_CONFIG); detectedProjectId = _fc && _fc.projectId ? _fc.projectId : detectedProjectId; } catch(e) { /* ignore */ }
          }
          if (!detectedProjectId && process.env.GOOGLE_APPLICATION_CREDENTIALS) {
            try { var sac = require(process.env.GOOGLE_APPLICATION_CREDENTIALS); if (sac && sac.project_id) detectedProjectId = sac.project_id; } catch(e) { /* ignore */ }
          }
        }
        if (!detectedProjectId) {
          console.warn('Skipping auto-creation of default admin: no Firebase project id or service account detected. Set GOOGLE_APPLICATION_CREDENTIALS or FIREBASE_CONFIG, or set ADMIN_DISABLE_AUTO_CREATE=true to suppress this.');
          return;
        }
      } catch (e) {
        // If detection fails for any reason, avoid throwing and continue to guarded logic below.
        console.warn('Project id detection failed; skipping admin auto-create guard check', e);
      }
...
// Admin: list users (returns merged users from Firestore). Caller must be an admin.
app.post('/api/admin/list-users', async (req, res) => {
  try{
    ...
    if(!isAdminClaim && !allowedByList){ return res.status(403).json({ error: 'forbidden' }); }

    const fdb = admin.firestore();
    const snap = await fdb.collection('users').limit(1500).get();
    const out = [];
    snap.forEach(doc => { const d = doc.data() || {}; out.push(Object.assign({}, d, { email: (d.email||d.emailAddress||'') })); });
