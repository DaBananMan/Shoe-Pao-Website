...
(function() {
  ...
  const arEnabledEl = qs('#prodArEnabled');
  const arQrEl = qs('#prodArQr');
  const arLinkEl = qs('#prodArLink');
  ...
  try { const crit = (p.criticalLevel !== undefined) ? p.criticalLevel : (p.critical_level !== undefined ? p.critical_level : ''); const critEl = qs('#prodCriticalLevel'); if (critEl) critEl.value = (crit !== undefined && crit !== null) ? String(crit) : ''; } catch(e){}
      ...
      // AR fields
      try {
        if (arEnabledEl) arEnabledEl.checked = !!(p.ar && p.ar.enabled);
        if (arQrEl) arQrEl.value = (p.ar && p.ar.qr) ? p.ar.qr : '';
        if (arLinkEl) arLinkEl.value = (p.ar && p.ar.link) ? p.ar.link : '';
        const arFields = qs('#prodArFields'); if (arFields) arFields.style.display = (arEnabledEl && arEnabledEl.checked) ? 'block' : 'none';
    try { setArQrPreview((arQrEl && arQrEl.value) ? arQrEl.value : ''); } catch(e) {}
      } catch(e) {}
    } else {
  ...
  try { const critEl = qs('#prodCriticalLevel'); if (critEl) critEl.value = ''; } catch(e){}
      ...
      // AR defaults on add
  try { if (arEnabledEl) arEnabledEl.checked = false; if (arQrEl) arQrEl.value = ''; if (arLinkEl) arLinkEl.value = ''; const arFields = qs('#prodArFields'); if (arFields) arFields.style.display = 'none'; setArQrPreview(''); } catch(e) {}
    }
  ...
  const arEnabledVal = qs('#prodArEnabled') ? !!qs('#prodArEnabled').checked : false;
  const arQrVal = qs('#prodArQr') ? (qs('#prodArQr').value || '').trim() : '';
  const arLinkVal = qs('#prodArLink') ? (qs('#prodArLink').value || '').trim() : '';
  ...
  p.pricing.original = pOrig; p.pricing.sale = pSale; p.pricing.cost = pCost;
      ...
      // AR fields
      try {
        if (arEnabledVal) p.ar = { enabled: true, qr: arQrVal || '', link: arLinkVal || '' };
        else delete p.ar;
      } catch(e) {}
  ...
  // AR fields for new product
  try { if (arEnabledVal) newP.ar = { enabled: true, qr: arQrVal || '', link: arLinkVal || '' }; } catch(e) {}
  ...
  // AR QR preview helper: set preview image or hide preview area
  function setArQrPreview(value) {
    try {
      const previewWrap = qs('#prodArQrPreview');
      const previewImg = qs('#prodArQrPreviewImg');
      ...
      // If value looks like a data URL or a http/https URL, use it directly
      ...
      state.products.forEach(p => {
        ...
        (Array.isArray(colorsArr) ? colorsArr : []).forEach(c => {
          ...
          out.push({
            ...
            ,
            // AR: include per-product AR settings so storefront/product view can use them
            arEnabled: !!(p.ar && p.ar.enabled),
            arQr: (p.ar && p.ar.qr) ? p.ar.qr : '',
            arWebfilter: (p.ar && p.ar.link) ? p.ar.link : ''
          });
        });
      });
  ...
  const bulkUnarchiveBtn = qs('#bulkUnarchive'); if (bulkUnarchiveBtn) bulkUnarchiveBtn.addEventListener('click', bulkUnarchive);
    ...
    // AR toggle in product modal: show/hide AR fields
    const prodArEnabled = qs('#prodArEnabled');
    if (prodArEnabled) {
      prodArEnabled.addEventListener('change', function(){
        try { const f = qs('#prodArFields'); if(f) f.style.display = prodArEnabled.checked ? 'block' : 'none'; } catch(e){}
      });
    }

    // AR QR file upload / preview handlers
    const prodArQrFile = qs('#prodArQrFile');
    const prodArQrInput = qs('#prodArQr');
    const prodArQrRemove = qs('#prodArQrRemove');
    if (prodArQrFile) {
      prodArQrFile.addEventListener('change', function(){
        const f = prodArQrFile.files && prodArQrFile.files[0];
        if (!f) return;
        const fr = new FileReader();
        fr.onload = function(){
          try { if (prodArQrInput) prodArQrInput.value = fr.result || ''; setArQrPreview(fr.result || ''); prodArQrFile.value = ''; } catch(e){}
        };
        fr.onerror = function(){ alert('Failed to read QR image'); };
        fr.readAsDataURL(f);
      });
    }
    if (prodArQrInput) {
      prodArQrInput.addEventListener('input', function(){ setArQrPreview(prodArQrInput.value.trim() || ''); });
    }
    if (prodArQrRemove) {
      prodArQrRemove.addEventListener('click', function(){ try { if (prodArQrInput) prodArQrInput.value = ''; setArQrPreview(''); } catch(e){} });
    }
