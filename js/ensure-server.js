  ...
  try{
    ...
    window.__shoepao_ensure_server = true;
        try{
          ...
          // Defensive handling: only force sign-out on explicit auth failures (401/403).
          // Transient network errors or server-side failures (5xx) should not log the
          // user out automatically — especially when the user is on an admin page.
          if(!resp){
            try{ if(typeof console !== 'undefined' && console.warn) console.warn('verify-session: no response from verify endpoint'); }catch(_){ }
            return;
          }
          if(resp.ok){
            ...
            // Only sign out on clear authentication failures
            if(status === 401 || status === 403){
              await signOutAndRedirect();
              return;
            }
            // For server errors (5xx) and other transient responses, don't sign out.
            if(status >= 500){
              try{ if(typeof console !== 'undefined' && console.warn) console.warn('verify-session: server error (will not sign out)', status); }catch(_){ }
              return;
            }
            // For other 4xx errors (rare), avoid an automatic sign-out — log and wait for
            // the next periodic check. This prevents accidental sign-outs on non-auth
            // client errors or rate-limiting responses.
            try{ if(typeof console !== 'undefined' && console.warn) console.warn('verify-session: unexpected verify response (will not sign out)', status); }catch(_){ }
            return;
          }
