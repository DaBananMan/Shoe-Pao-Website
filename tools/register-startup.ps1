...
try {
    ...
    try{
        # Start the task immediately so the server begins now for convenience
        Start-ScheduledTask -TaskName $taskName
        Write-Output "Started scheduled task '$taskName' immediately."
    }catch{
        Write-Warning "Failed to start scheduled task immediately: $_"
    }
} catch {
